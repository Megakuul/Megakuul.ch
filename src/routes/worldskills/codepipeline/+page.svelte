...
<svelte:head>
  ...
  <meta
    name="description"
    content="CodePipeline examples for CodeBuild, CodeDeploy, Lambda, ECR, ECS, EC2, S3, and CloudFormation."
  />
  <meta property="og:title" content="CodePipeline - Megakuul" />
  <meta
...
<div class="sticky top-0 z-30 border-b border-white/5 bg-[rgb(17,16,16)]/70 backdrop-blur-md">
  <div class="mx-auto flex max-w-350 flex-row items-center gap-3 px-4 py-3">
    ...
    <span class="shrink-0 text-lg font-bold">CodePipeline</span>
...
<div class="mx-auto flex w-full max-w-350 flex-row gap-8 px-4">
  ...
  <main class="min-w-0 flex-1 py-8">
    <header class="mb-10">
      <h1 class="mb-3 text-3xl font-bold sm:text-5xl">CodePipeline</h1>
      <p class="max-w-2xl text-slate-200/50 sm:text-lg">
        Source, build, test, approval, and deployment examples.
      </p>
    </header>
          ...
          {#each group.snippets as snippet}
            ...
            >
              <div class="flex flex-row items-start justify-between gap-3">
                <h3 class="text-lg font-semibold sm:text-xl">{snippet.title}</h3>
                <button
              ...
              {#if snippet.note}
                <p class="mt-2 text-sm leading-relaxed text-slate-200/45">{snippet.note}</p>
              {/if}
