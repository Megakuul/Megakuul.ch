...
<svelte:head>
  <title>Gadgets | Megakuul</title>
  <meta
    name="description"
    content="Lambda diagnostics, HTTP interception, stream analysis, and database operations."
  />
  <meta property="og:title" content="Gadgets - Megakuul" />
  <meta
...
<div class="sticky top-0 z-30 border-b border-white/5 bg-[rgb(17,16,16)]/70 backdrop-blur-md">
  <div class="mx-auto flex max-w-350 flex-row items-center gap-3 px-4 py-3">
    ...
    <span class="shrink-0 text-lg font-bold">Gadgets</span>
...
<div class="mx-auto flex w-full max-w-350 flex-row gap-8 px-4">
  ...
  <main class="min-w-0 flex-1 py-8">
    <header class="mb-10">
      <h1 class="mb-3 text-3xl font-bold sm:text-5xl">Gadgets</h1>
      <p class="max-w-2xl text-slate-200/50 sm:text-lg">
        Lambda diagnostics, HTTP interception, stream inspection, and database operations.
      </p>
    </header>
    ...
    {#each data.groups as group}
      <section class="mb-16">
        ...
        <div class="flex flex-col gap-6">
          {#each group.snippets as snippet, index}
            <article
              id={snippet.id}
              data-anchor
              class="apple-glass scroll-mt-24 rounded-2xl p-4 sm:p-5"
            >
              ...
              <details class="mt-3" open={index === 0 || snippet.code.length < 9000}>
                <summary class="cursor-pointer py-2 text-xs text-slate-200/40 select-none">
                  {snippet.code.split('\n').length} lines · show or hide code
                </summary>
                <div class="code-wrap mt-1 overflow-x-auto rounded-xl">
                  {@html snippet.html}
                </div>
              </details>
            </article>
